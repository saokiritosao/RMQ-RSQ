...
int N_TEST   = 5;
char FILE_NAME[20] = "test_rmq_logn_n.txt";
...
int getMin(int** ST1, int n, int* logs, int l, int r)
{
    ...
    return min(ST1[lo][l], ST1[lo][r - (1 << lo) + 1]);
...
int main()
{
    ...
    printf("wwe\n");
    int** ST = (int**) calloc(logs[SIZE_ARR] + 1, sizeof(int*));
    for (int i = 0; i <= logs[SIZE_ARR]; i++)
    {

        ST[i] = (int*) calloc(SIZE_ARR + 1, sizeof(int));
